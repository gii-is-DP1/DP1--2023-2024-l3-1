...
public class GameController {
    ...
    public ResponseEntity<Game> createGame(@Valid @RequestBody GameCreateDto gameCreateDTO) {
        ...
        if (currentPlayer.isPresent()) {
            return new ResponseEntity<>(this.gameService.createGame(gameCreateDTO, currentPlayer.get()),
                    HttpStatus.CREATED);
        } else {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    ...
    public ResponseEntity<Void> play(@RequestBody PlayRequestDto playRequestDto) {
        ...
        Optional<GamePlayer> op_current_game_player = this.gamePlayerService
                .getGamePlayerByUsernameAndGame(currentPlayer.getUsername(), current_player_game);
